...
function buildCountyRecords(populationRows, ruccLookup) {
  ...
  grouped.forEach((rows, fips) => {
    const row2020 = rows.find(row => row.year === 1);
    const row2025 = rows.find(row => row.year === 7);
    ...
    Object.keys(GROUPS).forEach(groupKey => {
      values[groupKey] = {
        start: row2020.values[groupKey],
        end: row2025.values[groupKey]
      };
    });

    records.push({
      fips,
      stateFips: row2020.stateFips,
      stateName: row2020.stateName,
      countyName: row2020.countyName,
      rucc: rucc.rucc,
...
function valueForCounty(row) {
  ...
  const change = values.end - values.start;
  ...
  return (change / values.start) * 100;
}
...
function showTooltip(event, feature) {
  ...
  const values = row.values[appState.group];
  const change = values.end - values.start;
  const percent = values.start === 0
    ? null
    : (change / values.start) * 100;
  ...
  tooltip
    ...
    .html(`
      ...
      2020: ${d3.format(",")(values.start)}<br>
      2025: ${d3.format(",")(values.end)}<br>
